...
export default function Home() {
  ...
  // --- FUNGSI HITUNG (Helper untuk digunakan di Effect dan UI) ---
  const hitungRataMaqro = (m) => {
      // 1. Hitung Nilai Tarkib & Makna
  ...
  const hitungRataPresentasi = () => {
      return (parseFloat(presentasi.penguji1 || 0) + parseFloat(presentasi.penguji2 || 0)) / 2;
  };
  ...
  useEffect(() => {
    ...
    const totalPresentasi = hitungRataPresentasi();

    // Rumus Akhir: (M1 + M2 + M3 + Presentasi) / 4
    const totalSemua = totalMaqro1 + totalMaqro2 + totalMaqro3 + totalPresentasi;
  ...
  const DetailModal = () => {
    ...
    const pres = hitungRataPresentasi();
          ...
          <div className="p-5 space-y-4">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between items-center p-2 bg-purple-50 rounded-lg">
                <span className="text-gray-600 font-medium">Rata-rata Presentasi</span>
                <span className="font-bold text-purple-700">{pres.toFixed(2)}</span>
              </div>
            </div>
            ...
            <div className="border-t pt-3 mt-2">
              <p className="text-xs text-gray-500 mb-1 text-center font-mono">
                ({m1.toFixed(2)} + {m2.toFixed(2)} + {m3.toFixed(2)} + {pres.toFixed(2)}) ÷ 4
              </p>
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 pb-28 font-sans">
      ...
      <div className="max-w-md mx-auto px-4 space-y-5">
        ...
        <div className="bg-white rounded-2xl shadow-lg border-2 border-gray-100 p-6">
          ...
          {activeTab === 4 && (
             <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
               <h2 className="text-xl font-bold text-purple-800 mb-6 pb-3 border-b-2 border-gray-100">Presentasi Fiqh</h2>
               <div className="space-y-4">
                <div className="bg-gradient-to-br from-purple-50 to-purple-50 p-5 rounded-2xl border-2 border-purple-300 shadow-md">
                  <InputScore label="Nilai Penguji 1" val={presentasi.penguji1} onChange={(v) => updatePresentasi('penguji1', v)} />
                </div>
                <div className="bg-gradient-to-br from-purple-50 to-purple-50 p-5 rounded-2xl border-2 border-purple-300 shadow-md">
                  <InputScore label="Nilai Penguji 2" val={presentasi.penguji2} onChange={(v) => updatePresentasi('penguji2', v)} />
                </div>
               </div>
               <div className="mt-6 p-6 bg-gradient-to-br from-purple-100 to-purple-50 rounded-2xl text-center border-2 border-purple-300 shadow-lg">
                 ...
                 <p className="text-5xl font-black text-purple-700">{((parseFloat(presentasi.penguji1||0)+parseFloat(presentasi.penguji2||0))/2).toFixed(1)}</p>
               </div>
             </div>
          )}
        </div>
